        ...
        <div class="article-content-part border-radius-default">
          <div class="article-header">
            ...
            <div class="article-publish-time clear-fix">
              <div class="publish-info-left-part float-left">
                活动开始:
                <span v-text="formatDate(activity.startTime)"></span>
                <div class="el-icon-view">
                  {{ activity.viewCount }}
                </div>
              </div>

              <div class="publish-info-left-right float-right">
                活动结束:
                <span v-text="formatDate(activity.endTime)"></span>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getActivityDetail(){
      portal.getActivityDetail(this.$route.query.id).then((res) =>{
        if(res.data.code === 200){
          this.userId = res.data.data.sponsorId;
    ...
    getActivityUserInfo() {
      ...
      portal.getActivityUserInfo(this.userId).then((res) =>{
        console.log("====>3."+res.data)
        this.user = res.data.data;
        this.club = res.data.data.club;
      })
    },
    getHotActivity(){
      portal.getHotActivity().then((res) =>{
        if(res.data.code === 200){
          this.hotActivity = res.data.data.list;
        }
      })
    }
